...
module.exports = {
    ...
    findAll: async function (criteria) {
        try {
            ...
            const salesTransactions = await SalesTransaction.findAll({
                where: criteria.where,
                include: [{model: Article, as: 'article'}, {model: Merchant, as: 'merchant'}, {model: Sale, as: 'sale'}, {model: SalesTransactionPayment, as: 'salesTransactionPayments',include: [
                        {
                            model: PaymentType,
                            as: 'paymentType',
                            attributes: ['name']
    ...
    getSoldeInitial:async function (criteria) {
        try {
           ...
           const totalReglements = await SalesTransaction.sum('totalMerchantPayment',{where: criteria.where});
